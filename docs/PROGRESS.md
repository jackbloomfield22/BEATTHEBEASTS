...
- *Needs the sim (not changed here; another session owns `src/sim`):* (1) a `dive` look doesn't put him down, so a diving catch in the open lies on the turf while the sim carries him on. The render gets him straight back up (`getup_prone`) as a stopgap. The fix is to treat it like the carrier's dive: `a.move = 'dive'`, `busy` about 0.4 s, and he's down where he lands. (2) catchLook returns `highPoint` for every GO UP call whatever the ball's height; `call === 'aggressive' && at.z > ~1.75` would match what the render now does. (3) `npm run check` fails two sim tests on this branch: the determinism golden hashes and the completion-rate band (72% against at most 68%). Both exercise only `src/sim`, which this work doesn't touch, so they come from the sim-side commits (878b21b's SECURE-down changes the outcomes); the goldens need re-pinning over there.
