...
- **M6.5 Gameplay pass:** in progress on `claude/m65-gameplay` (brief: `docs/M6_5_BRIEF.md`). Done so far: #8 (runs), #1–7, #9, #10 and #11's animation half (the carrier clips and their render wiring; the awareness half is sim work still to do); next are the carrier's read layer and the ten concept videos.
...
**#9 carrier controls.** Three moves at a time under the carrier, one word each and large, on 1, 2, 3 (pad X, Y, B). The sim picks them (`src/sim/moves.ts`) every 0.2 s from the nearest unblocked tackler in front of or beside him. Square in front (within 35° of his run) offers truck, spin and juke. From the side it offers juke, stiff arm and spin. In the open field (nobody within 6 yd) it offers the best move for the nearest man's angle, then dive and protect. Each set is ordered by the odds the tackle roll itself uses (`tackleOdds`, split out of `resolveTackle` with the RNG order unchanged, so the run and pass harnesses are identical), so slot 1 is always the likeliest to work. The move he's making lights up lime. Every move keeps its own letter key (Q W E R F C). Settings v7 moves a saved v6 layout off the number row, keeping any key the player rebound. Not yet watched in a game capture; that happens with the concept videos.

**Route breaks, found making the concept videos.** Tracing a receiver's heading through his route showed the breaks rounded off: a slant turned from 0° to −75° over 0.9 s, a post bent 30° over 0.3 s, and a smash corner went 0° to 45° over 0.57 s. Only breaks sharper than 45° planted (`PLANT_COS` 0.7); anything gentler, which covers the slant, post and corner, was the steer's arc. A route runner now plants every break of 25° or more (`PLANT_COS` 0.9), and the slant goes to −45° in one step at 0.58 s. On its own that handed the receiver a free yard or two at every break (completion 71.0%, ypa 9.1), because the defender still swung round on the arc. So a cover man now plants too: a man defender, or a zone defender carrying the receiver in his area, once he's read the break (`coverPlant` in `src/sim/ai.ts`). His speed through the plant follows breakCarry's shape and his Agility, with at most one plant every 0.35 s. Pass harness, before → after: completion 68.6% → 65.7% (the owner's band is 60–65), ypa 7.8 → 7.9, int 0.7% → 0.9%, 20+ 14.4% → 16.6% of completions, 40+ 4.1% → 5.7%. The deep tail was already above the NFL's and is a little higher now; that's the next thing to look at for the passing numbers. Route fidelity is unchanged (no route strays 2 yd), and so is the run harness.

**The ten concept videos** (`docs/screenshots/m6.5/<concept>.mp4`, recorded with `BTB_VIDEO=1 BTB_CONCEPTS=1 BTB_VIDEO_QUALITY=low BTB_VIDEO_W=960`, 20 fps). Each is a scripted play (`CONCEPTS` in `src/game/clips.ts`) found by `tools/sim/findconcepts.ts`: thrown on time off the break, open, a real gain. `tests/sim.test.ts` holds each one to that. *Finding them showed two real problems, both fixed.* First, the rounded route breaks (above). Second, a clip that played differently in the browser: the first recording's go was a drop and its screen a fumble, where Node has a 48-yard catch and an 11-yard gain. The runner had been live while the page loaded, and time before the snap changes the play. Clips are now held from setup and snap on tick 0 in both places. After the re-record, all ten outcomes match Node: slant +12, quick out +9, curl +10, go +48, post +27, corner +36, crosser +15, screen +11, back shoulder +19, scramble drill +12.

What I saw, watched as contact sheets from the frames:
- *Slant, quick out, curl:* snap, the three-step rhythm, the ball out as he clears. The catch-call prompt comes up with the ball in the air, then the three move options sit under him the moment he has it. The curl's SECURE catch settles facing the QB. The slant still comes a beat late off the break: its ball arrives about 1.2 s after the break, where a real three-step slant is 0.6–0.8 s. The receiver's three steps are quicker than the QB's drop and release, so that's a timing gap in the sim, not the script.
- *Go:* the best of the ten. The pass camera rides the ball down the sideline, the Z runs away from the corner, and the safety comes over too late. The ball drops in over his shoulder at about the 22, and then JUKE / DIVE / PROTECT come up as the last man closes.
- *Post and corner:* the plant is visible now, a clean change of line at the break instead of a bend. The corner's defender flips his hips and runs with it (the cover plant) but is a step behind, which is the point of the throw.
- *Crosser:* the X runs under the deep third and catches it going full speed; the run after the catch is short, with the underneath defender coming off to make the tackle.
- *Screen:* the back slips out, the ball comes over the rush, and he gets upfield among the blockers. From the broadcast camera the linemen releasing in front of him are a crowd rather than a convoy. That's the most confusing of the ten to read.
- *Back shoulder:* GO UP is called while the ball is still in the air, and he turns back to it with the corner on top. At broadcast distance the turn is small; the throw reads as a completed deep out more than a back-shoulder. The catch-call prompt sits over the receivers at the key moment and covers part of it. That's a HUD placement issue to look at: the prompt belongs lower when the target is near the middle of the screen.
- *Scramble drill:* the QB escapes right, the prompt changes to THROW ON THE RUN, and the receiver works back across the field to him and makes the catch coming toward the ball. It reads like the real thing.
- *Pops:* 8–15 spikes a video, the worst 38–62 rad/s. Every one is authored fast motion: the defensive linemen's get-off arm punch, the tackle's wrap, the rip, the throw. None comes from the new carrier or catch clips, or from a cut.
- *Not judged here:* the frame rate (these are offline renders at low quality) and the full broadcast look (960 wide, low quality).

**#10 steering.** *Diagnosis* (`tools/sim/cuts.ts`: a carrier at full speed in open grass, the arrow 45°, 90° or 180° off his run, Agility 60–99): a change of direction was only the steer's lateral-acceleration limit, which is cutAccel less 35% at full speed. So every turn was an arc of about 11 yd radius. A 90-Agility back took 1.05 s to turn 90° and carried 4.5 yd on down the old line; at 60 Agility it took 1.73 s and 7.4 yd. Even 45° took 0.75 s. That is the "slow drift", and it's why getting around a man was so hard. The controlled pace near a tackler didn't change any of it. *Fix:* the arrows' changes of direction of 30° or more at speed are now a planted cut (`plantCut` in `src/sim/play.ts`). Over the plant, his velocity goes straight from the old run to the new one at the exit speed, then the burst out of the cut takes him back up. Agility sets both the plant's length and the speed kept. With a tackler on him (the controlled pace), the plant is 15% shorter, so the cut is sharper, not slower. An arrow let go during the plant (a tap) still finishes the cut and runs on along it for 0.3 s. Every cut emits a `cut` move event (side, angle, length) for the animation. *After:*

| Agility | 45° | 90° (speed kept) | 180° |
|---|---|---|---|
| 60 | 0.25 s | 0.33 s (51%) | 0.45 s |
| 75 | 0.22 s | 0.30 s (55%) | 0.37 s |
| 90 | 0.20 s | 0.25 s (56%), 1.2 yd on the old line | 0.32 s |
| 99 | 0.18 s | 0.23 s (58%) | 0.28 s |

The brief's target, under 0.4 s for 90° at 90 Agility, is met at 0.25 s (`tests/cuts.test.ts`). Only the player's carrier cuts this way. I tried giving the AI carriers the same plant: its lane choices jump direction often enough that every jump became a speed-losing cut. Runs fell to a 2-yard median and 54% of carries went 1–3 yd, so I reverted it; the AI keeps the arc and the run harness is unchanged. Not yet watched in the browser.

**#5 catch animations.** The sim side (catchLook in `src/sim/passing.ts`, the `look` on the catch event, SECURE going down in traffic) is wired to a new set of clips, keyed in `tools/blender/lib/actions_m65.py` (technique in its docstring). Each has a `secure` event (the ball in the hands) and a `tuck` event. The render (`src/render/game/choreo.ts`) asks catchLook up to 0.9 s before the arrival, picks the clip and its side from where the ball is going relative to his run, and starts it so the secure frame lands on the arrival. On the catch event it switches clips if the final look changed (a full-body clip already under way plays on), and on `secureDown` it plays the go-down from the secure frame. From the secure frame to the tuck the ball sits in the hands (between the palms, or in the one hand), and over the last 0.12 s of flight the drawn ball bends onto the hands when they're within a metre of it (render only). The build passes all 146 of 146 gates. Pictures: `docs/screenshots/m6.5/catch-clips-sheet.jpg` (Blender, a ball drawn in the hands from the secure frame), `catch-clips-lab.jpg` (the runtime, in the Lab: reach, secure, tuck; overlays over the run at 6 m/s with the new "Overlay (replayed)" control) and `catch-in-game.jpg` (broadcast camera, the stick play with RUN and SECURE, and four verts caught over the shoulder). New capture specs: `BTB_CATCH=1` (Lab) and `BTB_CATCHGAME=1` (a scripted pass under each call), both honouring `BTB_PORT`.
- *RUN, `catch_hands_run` / `_low` (overlays):* the hands come out late, into the diamond at the chest or pinkies together below the belt, give a few centimetres, and the ball goes high and tight into the right arm. The legs are the gait's, so there's no hitch in the stride. In the game it reads: the hands meet the ball and it's tucked a stride later. From the broadcast camera the diamond and the pinkies look much the same; you only see "hands out" at that distance.
- *GO UP, `catch_high_point`:* a long, low gather step, a one-foot takeoff off the left with the right knee driving, about 0.45 m of hip rise, the hands at full length above the helmet at the top (frame 22), the ball pulled down on the way down, a two-foot landing that folds over the knees, and two steps back into the run. In the game the first capture showed the jump on a chest-high stick throw: he leapt over a ball arriving at his belt. So GO UP now leaps only when the ball arrives at the shoulders or higher (1.75 yd); lower, he attacks it with the hands. That makes the call invisible on a low ball, which is the honest thing, but see the sim note below. The flight legs are a touch symmetric at the top: the knee drive is clear on the way up, but both legs hang alike at the apex.
- *SECURE, `catch_body` (overlay, arms and upper spine) and `catch_body_down`:* elbows in, forearms up in a cradle, the ball into the chest, the chest folds over it and both hands cover it. In the game the cradle is visibly different from RUN (lower, arms closer to the body, the trunk curling). The go-down turns the right shoulder in over a braking step, puts the left knee down and folds forward onto the forearms, lying on the ball (`stance_down_prone_ball`, which the get-up starts from). It reads well side on; not yet seen in a game capture, because the stick play's SECURE catch had nobody within 2 yd.
- *Deep ball, `catch_over_shoulder_l/_r` (overlays):* the upper back turns toward the inside shoulder, the hands come up in front of the face with the palms to the sky, the ball drops in and goes to the tuck. On four verts from behind the play it looks like the real thing: running away, the hands come up late, the ball lands in them over the shoulder. The head turn comes from the runtime look-at (up to 70°) plus the spine turn in the clip. On a ball dropping straight over his head the look-at clamp stops a little short.
- *Low and away, `catch_dive_l/_r`:* the near foot drives, the body turns 35° toward the ball and lays out flat about 0.5 m off the turf with the arms long and together, the hands close at full extension, the ball is pulled into the chest before the landing on the forearms and chest, and he slides to a stop lying on it. The silhouette is the best of the set. The landing heading is the clip's `turn`: the animator's new `onTurn` turns his root at the hand-over so the lying pose doesn't snap.
- *Sideline, `catch_toe_tap_l/_r`:* the upper body leans out over the white with both arms high and outside, the catch on the inside foot, the outside foot taps on its toes, and the inside toe drags forward as he falls out, then he gathers himself at a jog. The first version barely leaned (the spine's side-bend sign was backwards; fixed by looking at it). The drag is subtle: at broadcast distance you see the lean and the arms, not the toes.
- *High and outside, `catch_one_hand_l/_r` (overlays):* the outside arm goes to full length high and outside, the trunk leaning with it, the ball is snatched in that hand, pulled to the chest, covered, then tucked (always into the right arm, which is the arm the game carries in). The left-hand version first reached over the helmet instead of outside (the same lean sign); it's fixed now.
- *What still looks off:* none of the full-body clips can follow the sim's actual speed, because the render places the body where the sim says (the clip plays in place). A high point on a receiver the sim has nearly stopped jumps in place, and the dive's slide is the sim's motion, not the clip's. The catches were judged on stills, the Lab and three scripted plays, not on a long run of real games. The dive, the toe-tap and the go-down have only been seen in Blender and the Lab.
- *Needs the sim (not changed here; another session owns `src/sim`):* (1) a `dive` look doesn't put him down, so a diving catch in the open lies on the turf while the sim carries him on. The render gets him straight back up (`getup_prone`) as a stopgap. The fix is to treat it like the carrier's dive: `a.move = 'dive'`, `busy` about 0.4 s, and he's down where he lands. (2) catchLook returns `highPoint` for every GO UP call whatever the ball's height; `call === 'aggressive' && at.z > ~1.75` would match what the render now does. (The two sim tests this branch saw failing, the goldens and the completion band, were already fixed on the gameplay branch before the merge.) Both sim notes are now done on the gameplay branch: a diving catch puts him down where he lands, 0.4 s after the catch (as the carrier's dive does), and GO UP calls `highPoint` only for a ball at 1.75 yd or higher, matching the render. The render's get-up now fires only if the sim ever leaves him up after the landing.

**#11 carrier animation and awareness.** A ball carrier now has his own motion, keyed in `tools/blender/lib/actions_m65_carrier.py` (technique and sources in its docstring) and blended by the sim's state. The build passes 164 of 164 gates, with new mechanics gates for the carrier gaits (hip height and trunk lean against the receiver's). The render side is in `src/anim` (the gait families, the dip layer, the press) and `src/render/game/choreo.ts` (`carrierDrive`, `playCut`, `dipFor`, `trafficOf`, `reachDive`). Nothing in `src/sim` changed, and the determinism golden is untouched. Pictures:
- `docs/screenshots/m6.5/carrier-clips-sheet.jpg`: Blender renders of the gaits side on, the cuts from the front, the truck, hurdle and dive, and the overlays.
- `carrier-clips-lab.jpg`: the runtime in the Lab at 6 m/s (receiver, carrier in space, traffic, drive, pressing, dip left and right, front and side), and the moves at their key frames.
- `carrier-in-game.jpg`: the scripted cut run from the broadcast camera, the same frames cropped, the field camera, and the staged hurdle.

New tools: the Lab's "Ball carrier" and "Dip before contact" controls (`carry=`, `dip=` in the hash), `BTB_CARRIER=1` (Lab captures) and `BTB_CARRIERGAME=1` (a scripted inside zone steered with the arrows, `cut-run` in `src/game/clips.ts`, with `BTB_CARRY_VIEW=field` and a staged `hurdle` case), and `tools/sim/findcarry.ts`.

What each piece is and how it reads:
- *The tuck* was raised 8 cm (`TUCK_R`, shared by every carry, catch and move clip). At 1.19 m the hand sat at the belt line and the ball rode on his belly. Now the nose is at the pec and the elbow is in at the ribs. It reads as "high and tight" from his right side in the Lab. From the broadcast camera you see a white glove at the chest, not the ball's angle.
- *Carry gaits* (`carry_jog/_run/_sprint`): the right arm is locked on the ball (an aimed arm fitted to the tuck, so it rides with the chest through the lean and bob). The free arm works cheek to hip. The hips are 5–7 cm lower and the trunk 5–7° further forward than the receiver's gait at the same speed (measured: run 0.85 m and 17° against 0.92 m and 10°). The stride is the same length: the long stride is the open-field run. Side on in the Lab it reads as a back running behind his pads. From the broadcast camera the difference from a receiver is real but modest: a lower, more hunched silhouette.
- *Traffic* (`carry_traffic_jog/_run`): about three quarters of the step at the same speed, so the cadence is higher. The base is wider, the hips 12 cm lower than a receiver's, the knees higher, and the free hand is up in front of the ball. It blends in by the sim's context pace (`carrierPace`: fully in with a free tackler at 1.2 yd, out by 2.5 yd), eased over about 0.2 s. It's the clearest of the gait changes in the Lab. In the game it only lasts the half-second a tackler is close, so it reads as him gathering his feet, not as a distinct gait.
- *Pressing the hole*: on a designed run until he's a yard past the line, the trunk pitches 7° further forward and traffic is half in, so the steps are patient and choppy. It's subtle in the game.
- *The burst* (`carry_drive_*`): a longer ground contact at the same speed, the foot landing under the hips, full extension at toe-off, a low heel recovery and a 23–25° lean, for the length of the sim's `a.burst`. Side on in the Lab it's the most different of the gaits. In the game it reads out of the cut (the cropped frames at 2.5–2.6 s): he drives away low in a long stride.
- *Plant-and-cut* (`cut_plant_l/_r` for 30–75°, `cut_plant_sharp_l/_r` for 75° and more), on the sim's `cut` event:
  - The outside foot plants wide and ahead with the toe turned in, the hips drop (11 cm light, 22 cm sharp), and the trunk leans in toward the new line.
  - The shoulders turn to the new line before the hips, then he pushes off and the inside foot steps out along it.
  - The clip is authored in his own frame with no net turn. The game turns his heading with the sim's cut, and the foot lock pins the plant foot while the body turns over it.
  - It's timed so its plant spans the sim's (`rate = plant span / dur`, 0.6–1.8×) and starts just before the plant foot lands.
  - In the scripted play the 112° cut plays the sharp plant and the 45° one the light one. From the broadcast camera, cropped, the plant reads: low, wide, leaning left. At full frame it's a few pixels of change inside a crowd.
  - An AI carrier whose run turns faster than 2.5 rad/s at over 5 yd/s plays the light cut (once per 0.8 s). I haven't seen that in a capture.
- *The dip* (`ovl_dip_l/_r`): a weighted overlay of the arms and upper spine. The pads flex and bend down toward a free tackler closing inside 1.6 yd within 60° of his run (fully in at 0.9 yd), and the free hand covers the nose of the ball. The legs keep their stride. It reads in the Lab. In the scripted play I couldn't pick it out: the moment is a few frames long and inside the pile. It's covered by the unit tests, not by a picture.
- *The truck* is now full body at speed (`truck`; the old overlay is kept for a truck from a near standstill). The pads drop in the gather, the off forearm comes across as a shield and then up and through at the contact frame, and the legs keep driving in short, wide steps back into the run. In the Lab the forearm reads. The pad level is lower than the old overlay but still not as low as a real back's. Only seen in Blender and the Lab.
- *The hurdle*: a long gather step, a one-foot takeoff, about 0.26 m of hip rise over a 0.46 s flight, the lead knee up and the trail leg folded, the ball tight, the free arm out, landing on the lead foot in stride. It fires once per downed man whose body he'll cross 0.3–0.62 s ahead at over 5 yd/s, timed so the top of the flight is over the body.
  - In the Lab it reads as a leap. The lead leg doesn't reach forward enough at the top, so it's closer to a tuck jump than a hurdler's lead leg.
  - In the game it can hardly happen: over 936 AI runs, a carrier's path at speed crossed a man on the turf once. So the capture stages one: after the first cut, a defender is laid in his path, which is capture only.
  - It fires and plays in the right place (the log), but from both cameras the carrier is in a knot of bodies and the jump isn't legible.
- *The dive* near the goal line or the line to gain (the ball's nose within 2.2 yd of either) plays `dive_reach`: the ball arm goes to full length in front with the ball in the hand (the hold overlay lets go, and the drawn ball sits in the fingers), the free arm reaches down for the landing, and he lies with the ball still out (`stance_down_prone_reach`). Only seen in Blender and the Lab. The AI never dives at a line (only QBs slide), so in the game it's the player's dive only.
- *Eyes*: the look-at goes to the nearest free tackler in front of him or beside him (`threatOf`) inside 9 yd. Otherwise it scans upfield, a slow ±20° sweep, within the look-at's clamps. At broadcast distance a helmet turn is hard to see. It's there in the Lab.
- *The catch hand-over*: the carry weight comes in as the catch clip tucks the ball (the hold overlay starts at the same moment), so a receiver becomes a carrier over about 0.3 s, eased. This is by construction and not yet seen in a capture: the scripted play here is a run.

What's still off:
- The carrier set is right in the Lab and the Blender sheets, but at broadcast distance most of it is subtle. The cut and the burst read, cropped. The dip, the eyes, the press and the hurdle don't read at full frame. That's partly the camera's distance and partly the pile around a runner on an inside zone.
- The sharp cut's clip is 0.8 s, and its tail (its own two acceleration steps) first covered most of the sim's burst. Now, when the sim bursts, the cut gives way to the drive gait 0.05 s after its push-off (`PlayerAnimator.release`, a 0.16 s fade). In the capture the clip lets go about 0.2 s into a 0.57 s burst, so the drive gait shows for the rest of it. The fade is quick enough that I saw no pop in the stills, but I haven't watched it at speed.
- Plants the sim makes over a stride don't know which foot is down. The clip always plants the outside foot, and the fade-in (0.14 s) covers the switch. In a slow Lab playback you can see the swap.
- The truck is still a touch tall at contact. The hurdle's lead leg is short at the top.
- Performance: the family blend samples only the families with weight, so only the carrier pays for the extra clips, and it allocates nothing per frame. A player now has 18 more clip actions at zero weight, and three skips those without evaluating them. I haven't measured the frame time on Medium on the target machine. It needs the perf screen on the owner's M1 Pro.

Needs the sim (not changed here):
1. Hurdles: the sim never blocks on a downed man and gives him no body length, so a carrier's path almost never crosses one (1 in 936 AI runs). For hurdles to happen, a downed player needs a body (a capsule along his fall) that the carrier's lane choice sees, and he'd pick hurdling over going round when it's in his path at speed.
2. The reach: AI carriers never dive for the pylon or the sticks. The brief's awareness half ("sees the first-down marker and reaches for it, sees the pylon and extends"; the sideline, the lane, the wall and the cutback) is sim behaviour and isn't done here. The render already reaches the ball out whenever the sim dives near a line.
...
**Gate media** (`docs/screenshots/m6/`): the relit locker room empty, half and full, the QB, OL and an empty stall, the pick panel, the video wall and the tunnel door, in both moods (`pregame-*`, `lightsdown-*`); `pick.mp4` (the reels, the camera to the stall, the dressing, the hologram); `walk.mp4` (the pull-back and the walk-out into the stadium); `drive.mp4` (one full drive: the Meanwhile cut, the calls, the snaps, the whistle); and the game screens `game-*.png` (the play call, a set with the crew in place, a result, a fourth-down card, a field goal set, the results screen).
...
- *The lighting and detail pass (after the owner's review: "the room is dark, only the video wall is lit").* The room now lights itself the way ref-04 and the owner's second reference do, from self-lit surfaces rather than only the stall lamps, so it reads on any GPU (and if a GPU ever rejects the stall-light shader, the room falls back to standard lighting instead of drawing black):
  - every stall edge-lit at all times (lime when bare, his position's color and brighter once dressed), a warm amber interior glow, and a dim plate that names the slot;
  - a screen over every stall: the slot waiting, dimly, or his number and name on his position's color with team and decade;
  - oak slat paneling with its own warm wash; a light plaster ceiling with the perforated panel (the C in lime holes), a lime cove all the way round where wall meets ceiling, and two rings of downlights pooling on a grey chevron carpet with the CONTENDERS mark;
  - cognac leather on the stall seats and the bench, the bench on an underlit plinth;
  - the video wall as a lit screen, the brightest surface in the room. Measured on the shots (mean linear luminance): wall 0.17–0.20; pregame ceiling 0.09 and floor 0.05 (a third to a half of the wall); the stall interiors 0.19 (they are the light source). Lights down drops the room (ceiling 0.04, floor 0.01) and keeps the stall edges, the screens, the cove and the wall.
  - The wide shot now fills the frame with the row of nine, a band of lit ceiling above and the carpet's mark below; the stall shot is a medium shot that keeps the neighbors, screens, ceiling, bench and floor in frame, with the stall right of the center stage; the wall shot rides high so its reels sit above the stage's readout.
- *Still weak in the room:* the bench and the video wall's console are plain dark blocks; the ceiling panel shows only as a band at the top of the wide shot; the carpet's chevrons are regular enough to read as a texture up close. The owner's real-GPU look is the judge; these shots are software-rendered.
...
- *The game screens read as a broadcast.* The set from behind the offense with the four officials in their spots, the result card over the whistle, the fourth-down card, and the results screen all look like the game they are. Reviewing them caught real bugs, now fixed: the play call sat under the score bug; the fourth-down card's options ran off its left edge; field-goal distances showed as raw decimals ("24.76 yards"); the kicking unit stood wherever it was built until the strike, so the first kick of a game had no snapper, holder or kicker in frame; the other nineteen stood where the last play left them; and a hold-to-kick shorter than one long frame was missed (the charge is now timed from the key events, not sampled per frame).
- *The kick* now sets the full field-goal look (the tight protection with wings, eight in the gaps, edges and a jumper) behind the snapper, holder and kicker, from a camera just above the kicker. But nobody moves except the three specialists: no get-off, no rush, no leap, no crowd reaction yet. The punt is still a text cut, not a picture.
- *The results screen* works (and was rebuilt; see the results follow-up below). The first screenshot's box score has every catch to one man because the scripted input always throws to read 1.
...
#### Draft follow-up: the numbers come out, and one spot for the eye
- *No numbers anywhere in the draft, in every mode.* The draft tests football knowledge. The pick list has no OVR and runs QB, RB, WR, TE, OL, then last name (Jr./Sr./II–V skipped); the tabs still filter and full slots stay dimmed. The Scouting card drops the OVR, the bars and values and the confidence label: it names his three best card attributes ("Strengths": Speed, Deep Route Running, Catching; ties in card order), shows his traits with plain-language reasons, and keeps the era line. Film Room shows name, position, team and decade only (an OL unit keeps its five names). The dressing lower third, the video wall's pick card and the Beasts page on the wall show no ratings either (the tier word stays). The Ratings Explorer, the results screen and the Scouting panel outside the draft keep the full numbers.
- *Plain-language why lines* (`src/engine/ratings/traits/plain.ts`): built from the same ranks the numeric line states, named by attribute: "elite run blocking for a tight end", "among the fastest receivers", "a fairly light workload for a running back and among the best running backs in yards per carry". `tests/draft-plain.test.ts` runs every trait of every player and unit in the snapshot through it and finds no digit.
- *The stage* (`.draft-stage`): Spin is now one large translucent button at the center of the frame, between the locker row and the CONTENDERS mark, with the round counter above it and the Enter / A prompt inside. While the reels turn it reads out the team and decade reels; with a pair on the wall it's the Draft confirm for the focused man; with the row full it's Walk out. Auto-Draft (R / Y) sits under it. Its height is one variable, `--stage-y` in `draft.css` (65% for now).
- *Checked on screen* (the locker shots and `pick.mp4`): at 65% the stage sits on the bench line between the row and the CONTENDERS mark in the wide shot, reads over the scene, and holds Spin, the reel readout, the Draft confirm and Walk out in one place. Long names shrink to fit the Draft button. The focused stall's shot puts the stall right of the stage, and the wall shot puts the reels above it.

#### Results follow-up: every game ends on its results, and the results stay
- *The bug (a Classic game ended with no results screen, and no way back to it).* Three causes, all fixed:
  1. **Esc froze the game.** In a game, Esc during a snap paused the play engine (the Practice Field's pause), but the game screen drew nothing for a paused snap and nothing listened for a resume: the field froze under the score bug for good. Pressing Esc during the last play's 1.6 s dead-ball hold (the natural "skip" key after the final whistle) stranded the final whistle, so the match never went final and the results never came. Reproduced in the browser before the fix (`tools/shots/out/results/repro-pause.png`). The game screen now owns the pause: Esc / Start brings a pause menu up over a snap or a card (the play call, fourth down, the try, the Meanwhile cut, the result card), with Resume and Leave game.
  2. **The results only lived in memory.** The results screen read the live match; once you left it (or reloaded, or anything went wrong) that game was gone. Now the game's record is built and saved to History the moment the match goes final (in the play engine's whistle, a kick, a punt or a kneel, before any navigation), and the results screen draws the saved record.
  3. **A press could skip the results unseen.** The results screen took Enter the moment it mounted, with Locker Room focused: a confirm pressed to hurry past the final banner landed you in the locker room. Now the results play out (the score counts up, the result and grade land, then the box score opens) and a press during that only skips ahead; the final banner says "Enter: Results".
- *Leaving from the pause menu* ends the game where it stands and opens its results. A snap whose whistle has already blown still counts, so leaving on the final play after its whistle is a proper final; leaving mid-game is kept as "Left early", ungraded, its open drive on the chart.
- *The box score* (`src/game/stats.ts`, built from each snap's sim state and events): passing (C/Att, yards, Y/A, TD, INT, sacks and yards lost, long, NFL passer rating, legacy grade); rushing per carrier (carries, yards, average, TD, long, broken tackles, fumbles and lost); receiving per man (targets, catches, yards, average, TD, long, YAC, drops, contested catches won of contested targets: a Beast within a yard at the catch point, the harness's in-phase line); the line per man (sacks and pressures allowed, run-block win rate: he holds his man for 2.5 s or the whistle on a designed run, after ESPN's RBWR); the Beasts per man (tackles, sacks, pressures, INTs, pass breakups, big hits) and the coverage snapshot (the Beast nearest your best receiver when the ball came out, over every dropback, with the targets and yards when he was on him). One sim addition, bookkeeping only: `PlayState.pressures` records each defender who got pressure (the harness's 0.45) and the blocker he beat. It isn't hashed and nothing reads it: the determinism hashes are unchanged.
- *The play of the game:* your touchdowns first, then turnovers, then the rest; within each the biggest expected-points swing (a linear fit to nflfastR's first-and-10 EP), half again late in a close game. It's stored with the snap's setup and your inputs (run-length encoded) and flagged for the M7 replay system.
- *The screens.* Results: tabs (Q/E, LB/RB) for Summary (play of the game, team line, key matchups, the drive chart with both teams' drives: start, plays, yards, result), Passing & Rushing, Receiving, O-Line and The Beasts; ↑↓ scroll a long panel. Numbers show in every mode, Film Room included: the game is over. **History** is live on the main menu: every game (the last 100), newest first, the focused one's leaders, play of the game and drive chart, Enter for its full results. The **Locker Room** entry shows a Last Game board (score, grade, team line, drive chart) left of the stage; F / X opens the full box score over the room.
- Also fixed on the way: spike and kneel on a two-point try (they turned it into a 2nd-down drive), the Beasts' drive yards on the Meanwhile line showing raw decimals, the between-snaps clock carrying over from the previous game, and "1 turnovers" (the team line pluralizes).
- *Tests:* `tests/results.test.ts` (every way a match ends gives a record: regulation, overtime, the two-minute clock, leaving; the box score adds up over real sim plays; passer rating; the play of the game). `e2e/results.spec.ts`: a full Classic game from the real draft flow (Auto-Draft) to the results, every tab, the Locker Room board and History; then the two-minute drill running out, overtime, and Esc → Leave game on the final play's whistle, all landing on their results.
- *Critique (from the screenshots in `tools/shots/out/results/`, 1280×720):* the tabs read as a broadcast stat sheet and fit without scrolling for a six-round game; the drive chart is the best part. The review caught and fixed: the drive chart's columns spread across the whole panel (they hug their headings now); the Beasts' drive yards as raw decimals ("30.3158…", in the chart and on the Meanwhile line); the Last Game board clipping "BEASTS" off the score; the box score over the locker room letting the video wall read through it (the scrim is near-opaque now); a loss grade in the winner's lime (red now); and the header's slide up after the grade, which could stall mid-screen over the tables (now a keyframe that always lands). Still weak: the Summary's lower half is empty on a short game; the Passing & Rushing tab is sparse (one passer, a few carriers) and could hold a chart; your TD drives show "Touchdown" without the try's points. The reveal's beats couldn't be judged in the software renderer (its frames are too slow to catch them); it needs a look on real hardware. Fixed after the merge: a receiver never thrown at now shows "never thrown at" with no verdict (a ratings edge isn't a result), and the score bug steps aside under the pause menu (it overlapped the pause header).
