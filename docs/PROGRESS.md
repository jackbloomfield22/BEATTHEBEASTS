...
- *The results screen* works, but a key matchup that was never thrown at shows a verdict ("not targeted · REC WON") from openness alone; it should say he got open and wasn't used. The screenshot's box score has every catch to one man because the scripted input always throws to read 1.
...
- *Not yet verified on screen:* built and tested in code only; the locker screenshots (`npm run shots`, locker spec) need a pass to set `--stage-y` against the reframed wide shot and to check the stage over the carpet mark.
