...
import { createPlay, DEF_CALLS, input, PLAYS, practiceRosters, stepPlay, type OffSlot, type SimPlayer, type SnapshotLike } from '../../src/sim/index.ts';
...
export function run(offense: Record<OffSlot, SimPlayer>) {
  ...
  for (let k = 0; k < N; k++) {
    const s = createPlay({ seed: 9000 + k * 7919, offense, defense: r.defense, play: PLAYS[k % PLAYS.length]!, def: DEF_CALLS[k % DEF_CALLS.length]!, los: 35, toGo: 10, user: true, difficulty: 'pro' });
    let shed = -1;
...
const report = (label: string, offense: Record<OffSlot, SimPlayer>) => {
  const { ts, firstShed } = run(offense);
  console.log(`${label.padEnd(28)} sack p10 ${q(ts, 0.1)} p50 ${q(ts, 0.5)} p90 ${q(ts, 0.9)} | first shed p50 ${q(firstShed, 0.5)}`);
};
