...
export function diagnosisSection(run: RatingRun): string[] {
  ...
  out(
    ...
    `- **Stint totals from corrections.** White GB 1990s: legacy held 111.5 sacks (his 1990s decade across teams); the correction sets 68.5 (Packers 1993–98). His 1990–92 Eagles seasons belonged to no stint (the legacy list has no PHI 1990s entry); round 2 added that stint from the cited table (\`data/augment/added_stints.json\`: 14, 15 and 14 sacks, first-team All-Pro 1990 and 1991): ${get('defense:reggie-white:PHI:1990s') ? `${who(get('defense:reggie-white:PHI:1990s')!)} is #${rank(get('defense:reggie-white:PHI:1990s')!)}, ${f1(get('defense:reggie-white:PHI:1990s')!.ovr.value)}` : 'not rated'}. Smith's BUF 1990s stint is 10 seasons (1990–99) and includes 1991 (1.5 sacks, injured) and 1999 at 36: ${f2(smith.inputs.stats.sacksPerGame!.v)} a game (share ${f2(sackShare(smith))} of the league team rate against Garrett's ${f2(sackShare(garrett))}), so his sack evidence is worth about half of Garrett's.`,
    `- **Measurables** (see 3): no 40, bench or jumps for either in any source we hold (round 2's search of their Wikipedia articles found no cited 40 either), so Speed and Agility are body priors: White ${attr(white, 'speed')} / ${attr(white, 'agility')} (PHI), ${attr(whiteGb, 'speed')} / ${attr(whiteGb, 'agility')} at 31–37 in Green Bay; Smith ${attr(smith, 'speed')} / ${attr(smith, 'agility')}. Pursuit (30% speed) and Finesse Moves (25% speed and agility) carry it: White GB Pursuit ${attr(whiteGb, 'pursuit')}, Finesse ${attr(whiteGb, 'finesseMoves')}.`,
