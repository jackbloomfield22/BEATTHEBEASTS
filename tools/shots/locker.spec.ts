...
for (const mood of MOODS) {
  for (const s of STATES) {
    ...
    test(`${s.name} · ${mood}`, async ({ page }) => {
      await page.goto(`/?screen=draft&shot=menu&nointro&seed=7&room=${mood}&lighting=${mood === 'lightsdown' ? 'night' : 'golden'}&quality=${process.env.BTB_QUALITY ?? 'medium'}&${s.q}`);
      ...
      await page.screenshot({ path: `tools/shots/out/locker/${mood}-${s.name}.png` });
    });
  }
}
